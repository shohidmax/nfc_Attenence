...
void animateSuccess() {
    ...
    for (int i=0; i<360; i+=20) {
        float angle = i * 0.0174533;
        display.drawPixel(64 + 20*cos(angle), 32 + 20*sin(angle), SSD1306_WHITE);
        display.display();
    }
    display.drawCircle(64, 32, 20, SSD1306_WHITE);
    display.drawLine(54, 32, 62, 40, SSD1306_WHITE);
    display.drawLine(62, 40, 78, 24, SSD1306_WHITE);
    display.display();
    delay(1000);
}
...
void animateFailure() {
    ...
    display.display();
    delay(200);
    display.drawLine(53, 21, 75, 43, SSD1306_WHITE);
    display.display();
    delay(200);
    display.drawLine(75, 21, 53, 43, SSD1306_WHITE);
    display.display();
    delay(1500);
}
...
void animateSaving() {
    display.clearDisplay();
    display.drawRoundRect(48, 20, 32, 24, 4, SSD1306_WHITE); 
    display.fillRect(52, 20, 24, 12, SSD1306_WHITE);
    display.fillRect(60, 32, 8, 10, SSD1306_WHITE);
    display.display();
    delay(400);
    for (int y = 5; y < 18; y+=4) {
        display.fillRect(50, 22, 28, 20, SSD1306_BLACK); // Clear inside
        display.drawLine(64, y, 64, y + 8, SSD1306_WHITE);
        display.drawLine(64, y + 8, 60, y + 4, SSD1306_WHITE);
        display.drawLine(64, y + 8, 68, y + 4, SSD1306_WHITE);
        display.display();
        delay(150);
    }
}
...
void handleData() {
    ...
    doc["lastUid"] = lastUid;
    doc["lastCardData"] = lastNfcData; // Send raw card data to web UI
...
void loop() {
  ...
  if (nfc.tagPresent()) {
    ...
    if (tag.hasNdefMessage()) {
        ...
        String data = "";
        // The first few bytes of a text payload are status/language bytes, so we skip them
        // The exact number of bytes to skip can vary, 3 is common for UTF-8 text.
        int langCodeLength = payload[0]; // Length of the language code, e.g., 'en' is 2
